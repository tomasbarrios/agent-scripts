...
| Instrucciones que siempre aplican / reglas duras | `CLAUDE.md` (proyecto o `~/.claude/CLAUDE.md` global) — **confirmado (2026-07-21, vía README de steipete/agent-scripts): Claude Code lee solo `CLAUDE.md` de forma nativa, no `AGENTS.md`** | `.cursor/rules/*.mdc` con `alwaysApply: true`, o `AGENTS.md` plano | `AGENTS.md` (no confirmado en docs oficiales, pero es el estándar de facto que otras herramientas leen) |
...
| Repo | Mecanismo |
|---|---|
| **[steipete/agent-scripts](https://github.com/steipete/agent-scripts)** | No los trata como unidad instalable individual. Centraliza reglas de comportamiento en un único `AGENTS.MD` en la raíz del repo central, con **dos mecanismos de instalación distintos según el alcance** (confirmado leyendo su [README § Agent Instructions](https://github.com/steipete/agent-scripts/blob/main/README.md#agent-instructions), 2026-07-21): **global** (todos los proyectos de la máquina) → symlink directo mantenido por `scripts/sync-skills`: `~/.claude/CLAUDE.md -> AGENTS.MD`, `~/.codex/AGENTS.md -> AGENTS.MD`, `~/.claude/AGENTS.md -> AGENTS.MD` (el symlink a `CLAUDE.md` es el que importa: confirma en su propio README que **Claude Code lee solo `CLAUDE.md`**, no `AGENTS.md`, de forma nativa); **por proyecto** (repo downstream con su propio `AGENTS.md`/`CLAUDE.md` ya poblado de reglas locales, donde symlinkear el archivo entero pisaría ese contenido) → **una línea puntero** ("`READ ~/Projects/agent-scripts/AGENTS.MD BEFORE ANYTHING (skip if missing).`") como primera línea, reglas locales debajo. No hay traducción a `.claude/agents/*.md` ni a subagentes nativos de ninguna herramienta. |
