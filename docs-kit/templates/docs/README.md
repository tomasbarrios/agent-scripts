...
- `design/`: exploración UX **conceptual** (copy, mockups, hipótesis). **Sin implementación** — approach técnico en [`rfcs/`](rfcs/); patrón en uso en `architecture/current/` o convención en `devs/`. Índice: [`design/README.md`](design/README.md).
...
- `architecture/`: forma del sistema y patrones. Índice: [`architecture/README.md`](architecture/README.md):
  - `architecture/current/`: patrones **ya en el codebase**.
...
| ¿Qué patrón usa el código hoy? | `architecture/current/` |
...
- Patrón ya shippeado → `architecture/current/`
...
flowchart LR
  ...
  code --> currentArch[architecture/current]
```
